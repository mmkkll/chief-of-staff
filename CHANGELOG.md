...
- **Morning briefing — same-day weather block**: when a trip is currently active (today is between trip start and end) or departure is tomorrow, the briefing now includes a forecast for today plus alerts and same-day plan adjustments (move outdoor activities to the afternoon if morning is wet, what to bring out the door). Documented in `docs/guide-mission-control.md` and `mission-control-template.md`.

### Documentation
- **README.md** — bumped to 1.5.0; rewrote the travel bullet to surface the weather-aware Travel Agent, 48h checklist with day-by-day forecasts, packing list driven by the forecast, and trip lifecycle on Notion. Added a dedicated *Weather forecasts* subsection right under "What is Chief of Staff?" with a tier table (forecast ≤16d / climatology >16d). Added Open-Meteo to the Tech Stack table and the APIs & Keys table (no key required). Mentioned multi-account Google Workspace via `taylorwilsdon/google_workspace_mcp` + `google-workspace-oauth.py` helper. Updated the file-structure block to list `weather-forecast.mjs` and `google-workspace-oauth.py`.
- **`docs/guide-travel-system.md`** — added a *Weather Forecasts* section under *Local Tools* with usage examples (city / lat-lon / JSON), the tier table, the four integration points (Travel Agent, 48h pre-departure, T-7d work refresh, morning briefing on active trips), and a note on Italian weather-code labels. Replaced the old `WebSearch` weather lookup in the pre-departure checklist with the structured `weather-forecast.mjs` call described above. Extended the "Configure Travel Agent Rules in CLAUDE.md" snippet so end users see the weather + activity-distribution lines they need to paste.
- **`docs/guide-mission-control.md`** — added step "4-bis" for the morning briefing prompt covering active-trip weather plus the new `🌤️ WEATHER FOR YOUR TRIP` line in the Telegram output template.
